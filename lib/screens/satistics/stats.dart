...
class _StatsPageState extends State<StatsPage> {
  ...
  final Map<DateTime, Map<String, double>> _assessmentScores = {};
  ...
  void _showDayScores(BuildContext context, DateTime date) {
    ...
    if (scores == null || scores.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No tests taken on this day')),
      );
      return;
    }
...
class OverallStatsPage extends StatelessWidget {
  ...
  const OverallStatsPage({super.key, required this.assessmentScores});
  ...
  int getTotalFlagsRaised() {
    int flags = 0;
    for (var tests in assessmentScores.values) {
